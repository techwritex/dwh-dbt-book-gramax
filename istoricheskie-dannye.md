...
В качестве источника для «снэпшотов» используются модели staging-слоя. На примере одной из исходных моделей проекта можно отследить изменение контактной информации заказчиков (`stg_pg__customers`).

### Настройка и построение модели «снэпшота» 

В качестве источника для «с