...
В случае использования стратегии проверки может возникнуть соблазн настроить для отслеживания изменений все столбцы исходной таблицы, указав `check_cols = «all»`. Хотя такая настройка является штатной, но разработчики dbt все-таки рекомендуют перечислять конкретные столбцы, которые необходимо проверить или же рассмотреть возможность использования суррогатного ключа по нескольким столбца. Затем уже его указывать в настройках формирования «снэпшота».
