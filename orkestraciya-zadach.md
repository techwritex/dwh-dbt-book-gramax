...
Создайте в папке `dags/` файл  `carsharing_general_job.py` и постепенно добавьте представленный далее код.

<note type="lab" title="Примечание">

Для написания Airflow DAG воспользуемся TaskFlow API, который упрощает написание кода рабочих процессов и делает его легче в обслуживании. Суть в том, что Вы пишете функции Python, декорируете их, а Airflow создает задачи, подключает зависимости и передачу данных между задачами.
