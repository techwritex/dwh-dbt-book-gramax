...
<image src="./orkestraciya-zadach-2.png" title="Рисунок 77. dbt-пайплайн проекта в Airflow UI" crop="0,0,100,100" objects="square,0.7987,31.6254,5.3915,13.0742,,top-left&square,9.485,19.0813,16.2743,8.4806,,top-left&square,9.3852,71.7314,23.463,8.8339,,top-left" width="1784px" height="1008px" float="center"/>

Процесс содержит задачи в заданной последовательности из файла `dbt_pipeline_carsharing.py`.

![](./orkestraciya-zadach-3.png "Рисунок 78. Задачи dbt-проекта"){width=1870px height=798px}

Так выглядит успешный результат выполнения рабочего процесса:

![](./orkestraciya-zadach-4.png "Рисунок 79. Успешный запуск dbt-проекта с помощью Airflow"){width=1852px height=802px}

Но прошу обратить внимание, что это учебный проект. Он достаточно просто и ломаться практически нечему. Но даже в такой реализации при возникновении ошибок появятся определенные трудности в отладке и поддержке этого решения.

Если возникнет ошибка при запуске хотя бы одной из моделей, то весь процесс свалится в ошибку. Потребуется время, чтобы выяснить место и причину ошибки. Конечно можно прописать в DAG более детально этапы процесса (например, отдельные этапы формирования каждой сущности или же каждой модели проекта). Это позволит более оперативно находить место ошибки, устранить ее и перезапустить задачу. Но подход оркестрации достаточно трудоемкий. Поэтому рассмотрите его просто в учебных ознакомительных целях. В продуктивных решениях используйте вариант с Astronomer Cosmos.

Теперь помимо основной функциональности dbt Core вы имеете представление об оркестрации dbt-задач и можете автоматизировать преобразование данных в хранилище. Смело продолжайте уже более детально разбираться с этим интересным и полезным инструментом. Небольшой [перечень дополнительных ресурсов](./dopolnitelnye-resursy) поможет вам сделать следующий шаг в этом направлении.