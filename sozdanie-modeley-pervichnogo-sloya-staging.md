...
Теперь, когда определены источники, можете наконец-то начать использовать функциональность dbt&trade; для преобразования данных.

Первым шагом в процессе преобразования будет создание моделей staging-слоя.

## Модели слоя

Как ранее было обозначено, одно из основных преимуществ dbt&trade; автоматизация повторного использования фрагментов кода с помощью шаблонизатора Jinja.

<note type="lab" title="Примечание">

Одна из наиболее важных функций dbt&trade; (реализованных с помощью Jinja) – `ref()`, которая позволяет связывать модели между собой. Есть еще одна функция, которая тоже выполняет подобную задачу, но используется для привязки источников данных к моделям – `source()`.
...
Итак, с помощью dbt&trade; вы создали объекты staging-слоя в вашей платформе данных. Но сам код до сих пор хранится только на вашей локальной машине. Загрузите текущее состояние dbt-проекта в GitHub-репозиторий.
