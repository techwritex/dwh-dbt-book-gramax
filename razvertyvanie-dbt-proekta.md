...
Все модели в dbt™ создаются с использованием обобщенных табличных выражений или common table expressions (CTE).
...
Папка `snapshots` содержит «слепки» (снэпшоты) таблиц для формирования исторических данных и отслеживания изменений.

В dbt™ снэпшоты реализуются согласно механизму медленно изменяющихся измерений (Slowly Changing Dimensions, SCD) второго типа (SCD2). При этом подходе сохраняется полная история изменений данных по измерению с добавлением новых строк в таблице при каждом изменении. При этом старые записи сохраняются.

Рассмотрим вкратце общий процесс создания снэпшотов в dbt™, который состоит из трех этапов:

1. **Начальная загрузка**: при первом запуске процесса dbt™ фиксирует и сохраняет текущее состояние исходной модели в таблицу снэпшота, создавая таким образом начальную версию для каждой записи.

2. **Обнаружение изменений**: при последующих запусках dbt™ оценивает каждую запись на основе заданной стратегии выявления изменений (временная метка или сравнение определенных столбцов). Если значение какого-либо столбца изменилось, dbt™ считает запись обновленной.
...
Таким образом, механизм снэпшотов в dbt™ помогает работать с отслеживанием исторических данных.

Подробнее процесс формирования снэпшотов представлен в разделе [История изменения данных](./istoricheskie-dannye).
