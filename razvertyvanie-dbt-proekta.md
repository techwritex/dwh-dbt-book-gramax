...
Перейдите в [GitHub](https://github.com/new) (учетную запись вы создавали ранее на [шаге подготовки оснастки](./trebovaniya/_index)) и создайте новый репозиторий, который будет содержать все файлы проекта и историю их изменений.
...
Оставьте на некоторе время GitHub и займитесь инициализацией dbt-проекта.
...
Первым делом расскажите dbt&trade; о том, к какой платформе данных вы собираетесь подключаться. Если ранее вы устанавливали только один адаптер, то в перечне платформ будет только одно значение. В моем примере две платформы, так как ранее тестировал несколько вариантов. Соотвественно, выберите номер платформы из списка.
...
Хотя мы еще не погружались в детали dbt&trade;, но здесь стоит немного задержаться, чтобы понять суть параметра `threads`.

В момент запуска проекта создается [направленный ациклический граф (DAG, инструмент моделирования или структурирования данных)](./sozdanie-modeley-promezhutochnogo-sloya-intermed#отслеживание-связей-между-моделями), построенный на связях между основными сущностями dbt&trade;, которые называются моделями (но по сути являются SQL-запросами).
...
Например, если указать `threads: 1`, то dbt&trade; создаст только одну модель и не перейдет к работе над следующей моделью, прежде чем не завершит работу над текущей. Указание `threads: 5` означает, что dbt&trade; будет работать над пятью моделями одновременно.
...
Как было отмечено ранее, данный файл содержит параметры соединения с платформой данных и хранится за пределами проекта по соображениям безопасности, поэтому он отсутствует в обозначенной структуре.
...
В первой строке указано имя профиля (carsharing). Этих имен может быть несколько, так как с помощью dbt&trade; можно подключаться к нескольким платформам данных.
...
Например, требуется выполнить специальный (ad-hoc) запрос для подготовки какого-то, скажем, разового отчета или что-то быстро протестировать (возможно, в части логики), но при этом нет необходимости в материализации результата запроса и периодичности его выполнения. Просто нужно один раз скомпилировать запрос функциональными возможностями dbt&trade;, скопировать полученный результат и затем использовать его, где это требуется.
...
Макрос в dbt&trade; -- это повторно используемый фрагмент кода, который можно вызывать в различных местах dbt-проекта. Макросы упрощают работу по преобразованию данных, снижают трудозатраты на разработку и сокращают количество ошибок.

Макросы в dbt&trade; создаются с помощью шаблонизатора (или языка создания шаблонов) [Jinja](https://jinja.palletsprojects.com/en/stable/templates/), который позволяет создавать динамические SQL-запросы со сложной логикой, включать в них переменные, а также использовать структуры управления, такие как условные операторы (`if/else`) и циклы (`for`).
...
Все модели в dbt&trade; создаются с использованием обобщенных табличных выражений или common table expressions (CTE).

</note>

### **seeds**

Папка `seeds` используется для хранения небольших CSV-файлов со статическими редко изменяемыми данными (например, какие-то справочники), которые можно загрузить с помощью dbt&trade; напрямую в хранилище. На «сиды» можно ссылаться так же, как на модели проекта и использовать в процессах трансформации данных.
...
В dbt&trade; слепки реализуются согласно механизму медленно изменяющихся измерений (Slowly Changing Dimensions, SCD) второго типа (SCD2). При этом подходе сохраняется полная история изменений данных по измерению с добавлением новых строк в таблице при каждом изменении. При этом старые записи сохраняются.

Рассмотрим вкратце общий процесс создания «снэпшотов» в dbt&trade;, который состоит из трех этапов:

1. **Начальная загрузка**: при первом запуске процесса dbt&trade; фиксирует и сохраняет текущее состояние исходной модели в таблицу «снэпшота», создавая таким образом начальную версию для каждой записи.

2. **Обнаружение изменений**: при последующих запусках dbt&trade; оценивает каждую запись на основе заданной стратегии выявления изменений (временная метка или сравнение определенных столбцов). Если значение какого-либо столбца изменилось, dbt&trade; считает запись обновленной.

3. **Создание версии**: при обнаружении изменения предыдущая версия записи помечается временной меткой окончания, а новая версия вставляется с новой временной меткой начала. Этот процесс продолжается с течением времени, создавая историю изменения данных.

Таким образом, механиз «снэпшотов» в dbt&trade; помогает работать с отслеживанием исторических данных.

Подробнее процесс формирования «снэпшотов» представлен в разделе [Исторические данные](./istoricheskie-dannye).

### **tests**

В папке `tests` хранятся SQL-файлы для тестирования объектов dbt-проекта: моделей, источников данных, «сидов» (CSV-файлов со статическими данными), слепков данных («снэпшотов»).

Более подробная информация представлена в разделе «[Тестирование данных](./testirovanie-modeley)».
