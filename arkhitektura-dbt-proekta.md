...
Разумеется, схемы создаются в базе не руками, а с помощью dbt&trade; в момент запуска проекта на основании настроек в `dbt_project.yml`.
...
Этими настройками мы указываем dbt&trade;, что все модели слоя первичных данных (в папке `staging`) нужно сохранить в схеме `staging` (`+schema: staging`) в виде представлений (`+materialized: view`).

По другим слоям аналогично.

Но есть один интересный нюанс.

Настройка `+schema: staging` создает так называемую «пользовательскую» схему (custom schema). Когда мы указываем название схемы БД для определенного слоя (например, все тот же `staging`), то ожидаем, что dbt&trade; создаст в БД схему именно с таким названием. Но дело в том, что dbt&trade; генерирует имя схемы, добавляя имя пользовательской схемы к имени целевой схемы (target_schema) вот по такому шаблону: `<target_schema>_<custom_schema>`.
...
Создайте в папке `macros` SQL-файл с содержанием из Рисунка 13:
...
Важно знать, что имя sql-файла должно соответствовать названию макроса. В данном случае, название макроса `generate_schema_name` и файла - `generate_schema_name.sql`.
...
В заключении данного раздела в папке `models` создайте 3 вложенные папки с названиями слоев хранилища (`staging`, `intermediate`, `marts`):
