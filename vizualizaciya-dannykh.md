...
Пройдемся по файлу подробнее. Это основной конфигурационный файл Lightdash.

`version: 1`

-  **Значение:** текущая версия схемы конфигурации Lightdash.

-  **Пояснение:** обязательный параметр, который гарантирует, что движок Lightdash будет интерпретировать последующие команды корректно. На данный момент используется версия `1`.

`projects`

-  **Значение:** список  проектов.

-  **Пояснение:** в одном репозитории теоретически могут быть несколько dbt-проектов, и здесь указываем описание конкретного аналитического пространства.

`- name: Carsharing Analytics`

-  **Значение:** имя проекта.

-  **Пояснение:** это  название проекта, которое будет отображаться в интерфейсе Lightdash (в выпадающем списке проектов). 

`dbt_project_dir: .`

-  **Значение:** путь к папке с dbt-проектом (относительно корня репозитория).

-  **Пояснение:** точка ( `.` ) означает, что файл `dbt_project.yml` лежит в той же папке, что и этот конфиг.  Lightdash использует этот параметр, чтобы найти основные настройки dbt-проекта -- манифесты и логику трансформаций.

`models:`

-  **Значение:** список фильтров (селекторов) для моделей.

-  **Пояснение:** параметр, который определяет, какие именно dbt-модели Lightdash должен импортировать и превратить в таблицы для анализа.

`- "marts/**"`

-  **Значение:** путь с маской.

-  **Пояснение:** эта строка дает команду: «Сканируй только папку `marts` и все её подпапки ( `/` )».

   -  **Почему это критично:** в dbt-проекте Carsharing много технических слоев: `staging`, `intermediate`. Если не указать этот параметр, Lightdash заберет в интерфейс все «технические» таблицы, которые только запутают бизнес-пользователя.

   -  **Результат:** аналитик (или бизнес-пользователь) увидит в списке доступных таблиц только витрины (например, `fct_payments` или `dim_cars`), а все остальные объекты останутся доступны только инженерам в исходном проекте.
