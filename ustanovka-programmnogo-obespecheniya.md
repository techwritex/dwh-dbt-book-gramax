...
<note title="Обновление dbt Core ">

В июне 2026 года компания dbt Labs анонсировала переход ядра dbt на язык Rust (dbt Core v2.0). Это революционное обновление уходит от Python-окружений в сторону быстрых бинарников и Parquet-артефактов.

Однако, на момент выхода новой версии, open-source коннектор (адаптер) для PostgreSQL еще находится в процессе миграции на новый движок. Поэтому в рамках учебного проекта, представленного в руководстве, используется стабильная Python-версия dbt 1.x.

Все архитектурные паттерны, концепции BI-as-Code и принципы построения хранилища, описанные в руководстве, остаются неизменными и полностью совместимы с грядущей v2.0.

Процесс перевода проекта на версию dbt Core v2.0 будет добавлен в руководство после выпуска разработчиком обновления коннектора для PostgreSQL.
