...
Подобные тесты конечно хороши, но неудобны тем, что написаны под конкретную модель или даже столбец. Но вполне может такое быть, что логика проверки одной модели может быть аналогичной и для другой модели. Разница будет только в названии этой самой модели и, возможно, имени столбцов. В этом случае тест можно с уверенностью сказать, что такой тест не будет единичным. Для таких целей нужно использовать общие тесты данных (generic tests).

### Общие тесты (generic tests)

Общие тесты по своей сути являются dbt-макросами.
