...
Ровно так же как и макросы, можно создать пользовательские тесты, если недостаточно функциональности встроенных или же требуется применить определенную логику на несколько объектов сразу.

Ранее вы внедрили проверку данных на отсутствие отрицательных сумм по оплате. Логика была применена к конкретной модели и конкретному полю этой модели. Но представьте, что есть и другие подобные модели, к которым тоже нужно применить такую же проверку. В этих  случаях используются пользовательские тесты.
