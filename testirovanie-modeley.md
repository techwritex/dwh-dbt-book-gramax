...
В данном разделе рассмотрим возможности dbt&trade; в части тестирования.

Тестирование в dbt&trade; – это выполнение определенных настроек в моделях, а также в других объектах проекта (источниках, CSV-файлах, «снэпшотах»), которые позволяют проверить целостность данных, а затем сообщить о результатах этих проверок.

В dbt&trade; тесты являются SQL-запросами, направленными на поиск «ошибочных» записей.
...
Данные в dbt&trade;  можно протестировать с помощью встроенной функциональности, а также с применением дополнительных библиотек («пакетов»).

В dbt&trade; существует два типа тестов:
...
-  `version: 2`. Эта настройка была обязательной для версии dbt&trade; 1.4 и ниже. Сейчас она не обязательна, но все-таки разработчики рекомендуют оставить ее;
...
Прямо из коробки dbt&trade; предоставляет возможность использования четырех встроенных универсальных тестов:
